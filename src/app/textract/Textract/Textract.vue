<template>
  <div :class="$style.textract">
    <button @click="fetchTextract">fetchTextract</button>
  </div>
</template>
...
export default {
  ...
  data: (): any => ({}),
  methods: {
    ...mapActions('textract',['fetchTextract','storeTextract']),
  ...
  computed: {
    breadCrumbItems() {
      return [
        ...
        { label: this.$t('common.Textract' /* Textract */), href: '/textract' },
      ];
    },
  },
  beforeCreate() {
    registerModule('textract', TextractModule);
  },
  prefetch: (options: IPreLoad) => {
     ...
     *
     * return options.store.dispatch('fetchTextract', '1');
     *
